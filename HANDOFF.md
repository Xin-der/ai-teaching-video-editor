...
> 最后更新: 2026-08-04 | 状态: v4 P1-P3 全部完成 + 前端多页面重构（用户手动，UI/功能修复待办见下）
...
**当前阶段**: v4 P1（前端编辑风重设计）+ P1.1 核心修复（缓存隔离 + 输入泛化）+ P2 帧点评（自动挑问题帧 + VLM 诊断 + 标注框）+ P3 选题灵感（精选库 + AI 生成更多 + 轻量行业配置）全部完成（2026-08-04）。用户随后手动将前端重构为**多页面结构**（/、/optimize、/topics、/history、/plan/<id>），UI/功能修复清单见第二章末。**注意：重构后测试未同步——test_frontend 3/13、test_advisor 17/18 待修复**。此前 v3.2（2026-08-02）ASR 抗噪升级：SenseVoice-Small + fsmn-vad + 驾考热词替换 paraformer，车内噪音场景转录从"碎片化不可读"提升到"成句可读、考试播报干净"；CPU 推理 ~38× 实时，完整 optimize 流程端到端验证通过。
...
### v4 前端多页面重构（用户手动，2026-08-04 晚）→ 修复待办

用户将 P1 的"单页双 Tab"重构为**多页面结构**（更贴近真实产品站；`web/app.py` 与前端模板由用户手动改，本次会话未改）。**注意：测试未同步，见待办 6。**

- `web/templates/base.html` 基础模板（Nav：优化 / 选题库 / 历史 / 开始使用）
- 独立页面：`/`（价值主张 + 示例卡）、`/optimize`（优化工作台）、`/topics`（选题库）、`/history`（历史列表）、`/plan/<plan_id>`（方案详情，读 `output/optimize/plan_<id>.json`）
- `web/app.py` 新增页面路由 + `/api/plans` + `_plan_to_blocks()`（Plan→Blocks 内容模型，新产出类型在 `PLAN_BLOCK_DEFS` 注册）+ 生成方案后写 JSON 供详情页/历史页
- JS 拆分为 `common.js / render.js / optimize.js / topics.js / history.js / plan.js`（原 `app.js` 删除）；`common.js` 提供 toast/copy/esc/fmtTime
- **行为变化**: 生成方案后自动跳转方案详情页（可复制/下载/回看）；历史方案存服务端 `output/optimize/plan_*.json`（不再是 localStorage）

**UI/功能修复清单（2026-08-04 用户提出，任务 #8-14）**：
- [ ] **选题页·AI 生成置顶**: `topics.html` "AI 生成更多"区移到页面最顶部并视觉强化，精选库移其后
- [ ] **选题页·留空校验**: `topics.js` 生成需 城市/季节/热点 至少一项，全空禁用按钮 + toast 提示
- [ ] **选题页·分类筛选**: 精选库顶部加分类筛选条（全部 + 各 category，从 /api/topics 动态提取）
- [ ] **优化工作台·表单控件**: `optimize.html` 拖放区/输入框/下拉重新设计（拖拽高亮、图标、上传中态）；默认自研 CSS 不引组件库（零构建约束，如需组件库需用户确认）
- [ ] **主页·查看完整方案**: `index.html` "查看完整方案"当前 `href=/history` 跳历史列表是 bug → 改为就地展开完整 5 块示例方案（不跳转）
- [ ] **测试修复**: 多页面重构后 `test_frontend` 3/13、`test_advisor` 17/18（断言仍针对旧单页结构/旧文案）——需适配新路由与 JS 拆分
- [ ] **全量验证 + 文档更新**: 单测全绿 + HTTP 冒烟 + 更新本档
...
- [ ] 前端多页面重构后的 UI/功能修复（AI 生成置顶 / 留空校验 / 分类筛选 / 表单美化 / 主页示例展开 / 测试修复）→ 清单见第二章
...
方向已定：驾校内容优化工具 → 内容教练工作台（v4，行业可配置·驾考先行；模块化 + 内容教练叙事）。已完成：v3 内容优化 + v3.1 前端 + v3.2 ASR 抗噪 + v4 P1 前端编辑风重设计 + v4 P1.1 核心修复 + v4 P2 帧点评 + v4 P3 选题灵感（详见第二/三/五章）。用户已将前端手动重构为多页面结构（/、/optimize、/topics、/history、/plan）。
下一步：先处理多页面重构后的 UI/功能修复清单（AI 生成置顶 + 留空校验 + 分类筛选 + 表单美化 + 主页示例展开 + 测试修复，任务 #8-14，详见 HANDOFF 第二章）→ 再进 P4 部署 + 运维手册。真实教练试用（docs/trial/）与反馈循环可并行推进。
