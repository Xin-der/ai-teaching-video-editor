...
> 最后更新: 2026-08-04 | 状态: v4 P1 前端编辑风重设计完成 + P1.1 核心修复（优化缓存隔离 · 输入泛化）
...
**当前阶段**: v4 P1（2026-08-04）前端编辑风重设计完成（营销前置 + 双 Tab 工具区）+ **P1.1 核心修复**（优化工作目录按 run 隔离，修"新视频出旧内容"；输入泛化：任意内容按实际主题出方案、过短内容友好提示；单测 顾问 13/13 + 前端 11/11 + ASR 10/10）。此前 v3.2（2026-08-02）ASR 抗噪升级：SenseVoice-Small + fsmn-vad + 驾考热词替换 paraformer，车内噪音场景转录从"碎片化不可读"提升到"成句可读、考试播报干净"；CPU 推理 ~38× 实时，完整 optimize 流程端到端验证通过。
...
### 已知 UI 布局/设计问题（前端审查，2026-08-04）
- [ ] **结果块布局失衡（主要）**: 结果块复用营销 3 列网格（80px/1fr/1.1fr），"复制"按钮列占 1.1fr，右侧大片空白失衡 → `web/static/js/app.js` `planCard()` 与 `web/static/css/style.css` `.block`
- [ ] **移动端导航不可达**: `@media(max-width:760px)` 中 `.nav-links{display:none}`，移动端无汉堡菜单，"功能/流程/选题"锚点不可达
- [ ] **手机/局域网访问复制失效**: 非安全上下文（http://局域网IP）下 `navigator.clipboard` 不存在 → 复制提示失败，需 `execCommand('copy')` 降级
- [ ] 平台下拉仅"抖音"一项（占位，等 P3 行业配置）
- [ ] 结果字段 `.field .k` min-width:72px，5 字标签（如"私信开场白"）换行错位

### 局域网访问注意事项（2026-08-04 用户改 0.0.0.0，未改动）
- [ ] **硬编码 IP 有误**: `run.py` 写死 `192.168.0.101`，本机实际 IP 为 `192.168.0.102` → `webbrowser.open` 会自动打开错误的地址连不上，建议改动态探测本机 IP
- [ ] Windows 防火墙需放行 5000 端口入站，否则手机连不上
- [ ] `0.0.0.0` 暴露无鉴权 API，局域网内任意设备可 POST `/api/optimize` 消耗 API 额度（试用阶段可接受，需知晓）
- [ ] `web/app.py` `--host` help 文案仍写"默认: 127.0.0.1"，实际默认已改 0.0.0.0（过时文案）
...
方向已定：驾校内容优化工具 → 内容教练工作台（v4，行业可配置·驾考先行；模块化 + 内容教练叙事）。已完成：v3 内容优化 + v3.1 前端 + v3.2 ASR 抗噪 + v4 P1 前端编辑风重设计 + v4 P1.1 核心修复（优化缓存隔离 · 输入泛化，详见第二/三/五章）。
