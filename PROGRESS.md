...
> 最后更新: 2026-08-04 | 状态: v4 P1-P3 完成 + 前端多页面重构（用户手动，UI/功能修复待办见下）
...
**下一步**: 先做前端多页面重构后的 UI/功能修复清单（AI 生成置顶 / 留空校验 / 分类筛选 / 表单美化 / 主页示例展开 / 测试修复，任务 #8-14，详见 HANDOFF.md 第二章）→ 再进 P4 部署 + 运维手册。真实教练试用（docs/trial/）与反馈循环可并行推进。
...
### v4 前端多页面重构（用户手动，2026-08-04 晚）→ 待修复
用户将"单页双 Tab"手动重构为**多页面结构**（`web/app.py` 与模板由用户改，本次会话未动）：
- 页面：`/`（价值主张+示例卡）、`/optimize`、`/topics`、`/history`、`/plan/<plan_id>`（方案详情，读 `output/optimize/plan_*.json`）；`base.html` 基础模板
- `web/app.py` 新增页面路由 + `/api/plans` + `_plan_to_blocks()`（Plan→Blocks）+ 生成后写 JSON；历史方案从 localStorage 改为服务端 JSON
- JS 拆分为 `common/render/optimize/topics/history/plan.js`（删 `app.js`）；生成后自动跳转方案详情页
- **测试未同步**: `test_frontend` 3/13、`test_advisor` 17/18 待适配
- **UI/功能修复待办**（任务 #8-14，详见 HANDOFF.md 第二章）：AI 生成置顶、留空校验、分类筛选、表单控件美化、主页示例就地展开（当前跳历史是 bug）、测试修复
